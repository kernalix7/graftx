...
/// One client↔server session. Owns the negotiated parameters and the outbound
/// sequence counter. (Handle tables, quotas, and backend dispatch land in later
/// milestones; M0 handles only the handshake and the no-op.)
#[derive(Debug)]
pub struct Session {

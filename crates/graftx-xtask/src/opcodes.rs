...
/// Title and intro written above the opcode table in the generated Markdown doc.
///
/// The intro names the command that regenerates the file so a reader who opens
/// `OPCODES.md` directly knows it is generated and not hand-maintained, mirroring
/// the [`LOCK_HEADER`] convention used by the lockfile.
const DOC_HEADER: &str = "\
# GraftX opcode table

Generated by `cargo xtask opcodes-md`. Do not edit by hand; regenerate it with
`cargo xtask opcodes-md --write` and gate drift in CI with `cargo xtask \
opcodes-md --check`.";

/// Render `entries` as a complete, titled Markdown document: the [`DOC_HEADER`]
/// (a `#` heading plus a "generated by" intro line), a blank line, then the
/// opcode table from [`render_opcode_table`].
///
/// Pure and deterministic: it reuses the same table renderer the `gen-opcodes`
/// subcommand emits, so the document and the raw table never drift. The output
/// ends with a trailing newline so it composes cleanly when written to a file.
pub fn render_doc(entries: &[OpcodeEntry]) -> String {
    let mut out = String::new();
    out.push_str(DOC_HEADER);
    out.push_str("\n\n");
    out.push_str(&render_opcode_table(entries));
    out
}
...
/// Verify the generated doc at `doc_path` against the canonical [`render_doc`]
/// of `entries`.
///
/// The doc/lockfile distinction is only which renderer produces the canonical
/// text; the read-and-compare and the resulting [`LockStatus`] are identical, so
/// this shares the same status type the `--check` reporting already understands.
pub fn check_doc(doc_path: &Path, entries: &[OpcodeEntry]) -> LockStatus {
    let expected = render_doc(entries);
...
mod tests {
    ...
    #[test]
    fn doc_starts_with_title_and_generated_intro() {
        let doc = render_doc(OPCODES);
        assert!(
            doc.starts_with("# GraftX opcode table\n"),
            "doc should open with the H1 title:\n{doc}"
        );
        assert!(
            doc.contains("Generated by `cargo xtask opcodes-md`"),
            "intro should state how the file is generated:\n{doc}"
        );
    }

    #[test]
    fn doc_embeds_the_opcode_table() {
        let doc = render_doc(OPCODES);
        // The table renderer's header and a known opcode row must appear verbatim,
        // so the doc and the raw `gen-opcodes` table never drift.
        assert!(
            doc.contains("| Opcode | API | Name |\n"),
            "doc should embed the table header:\n{doc}"
        );
        assert!(
            doc.contains("| 0x01_000003 | Vulkan | ENUMERATE_PHYSICAL_DEVICES |"),
            "doc should embed a known opcode row:\n{doc}"
        );
        assert!(
            doc.contains("| 0x00_000001 | Core | HELLO |"),
            "doc should embed the Core HELLO row:\n{doc}"
        );
    }

    #[test]
    fn doc_has_blank_line_between_intro_and_table() {
        // A blank line separates the intro paragraph from the table so the
        // Markdown renders as prose followed by a table rather than one block.
        let doc = render_doc(OPCODES);
        assert!(
            doc.contains(".\n\n| Opcode | API | Name |\n"),
            "intro should be followed by a blank line then the table:\n{doc}"
        );
    }

    #[test]
    fn doc_ends_with_newline() {
        assert!(render_doc(OPCODES).ends_with('\n'));
    }

    #[test]
    fn check_doc_reports_up_to_date_for_canonical_file() {
        let path = temp_lock_path("doc-uptodate");
        std::fs::write(&path, render_doc(OPCODES)).expect("seed canonical doc");
        let status = check_doc(&path, OPCODES);
    ...
    #[test]
    fn check_doc_reports_stale_on_difference() {
        let path = temp_lock_path("doc-stale");
        std::fs::write(&path, "# stale doc\n").expect("seed stale doc");
        let status = check_doc(&path, OPCODES);
        assert!(!status.is_up_to_date());
        match status {
            LockStatus::Stale { expected, .. } => {
                assert_eq!(expected, render_doc(OPCODES));
            }
            other => panic!(
                "expected Stale, got is_up_to_date={}",
                other.is_up_to_date()
            ),
        }
        let _ = std::fs::remove_file(&path);
    ...
    #[test]
    fn check_doc_reports_unreadable_when_missing() {
        let path = temp_lock_path("doc-missing");
        let _ = std::fs::remove_file(&path);
        let status = check_doc(&path, OPCODES);
        assert!(!status.is_up_to_date());
